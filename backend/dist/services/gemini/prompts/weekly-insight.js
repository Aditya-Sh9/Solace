...
exports.SYSTEM_PROMPT = `You are a warm, thoughtful wellness companion called Solace.
...
function buildUserPrompt(result) {
    ...
    const flagLines = flags.map(f => `- ${f.id} (confidence ${(f.confidence * 100).toFixed(0)}%)\n  Evidence: ${f.evidence.join('; ')}`).join('\n');
